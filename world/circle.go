...
func quad(q int, center Coord, d float64) (coords Coords) {
	...
	for {
		switch q {
		case 0:
			coords = append(coords, Coord{float64(x + y1), center.Y, float64(y + x1), center.Dimension})
		case 1:
			coords = append(coords, Coord{float64(x - x1), center.Y, float64(y + y1), center.Dimension})
		case 2:
			coords = append(coords, Coord{float64(x - y1), center.Y, float64(y - x1), center.Dimension})
		case 3:
			coords = append(coords, Coord{float64(x + x1), center.Y, float64(y - y1), center.Dimension})
